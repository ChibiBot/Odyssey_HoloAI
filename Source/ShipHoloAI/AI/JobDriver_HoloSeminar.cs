...
namespace ShipHoloAI
{
    ...
    /// HoloAI_Lecture interaction for the bubble and play log. The seminar is
    /// downtime well spent, not downtime lost: attending grants recreation, and a
    /// student who was meditating keeps — and sharpens — their focus.
    /// </summary>
    public class JobDriver_HoloSeminar : JobDriver
    {
        private const int LectureDurationTicks = 300;
        private const float SeminarXP = 300f;
        // Roughly a quarter of a full joy activity (vanilla sessions land ~0.4);
        // with the 3-hour per-colonist cooldown this makes her lectures a
        // recreation supplement, not a joy fountain.
        private const float SeminarJoy = 0.1f;
        // ~2 in-game hours of meditation-spot psyfocus, so a lectured meditator
        // comes out ahead of being left alone rather than behind.
        private const float SeminarPsyfocus = 0.05f;
        ...
        internal static void FireSeminar(Pawn avatar, Pawn student)
        {
            ...
            // Attendance counts as recreation (Social — it is, after all, a
            // conversation, however one-sided), and a meditating listener gets
            // psyfocus instead of a broken session: she times her cadence to
            // the mantra rather than talking over it.
            student.needs?.joy?.GainJoy(SeminarJoy, JoyKindDefOf.Social);
            if (student.CurJobDef == JobDefOf.Meditate)
            {
                student.psychicEntropy?.OffsetPsyfocusDirectly(SeminarPsyfocus);
            }
