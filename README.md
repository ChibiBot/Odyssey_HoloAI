...
The `Source/` directory contains a C# project targeting `net48`.

### Building

1. Install the .NET SDK (6.0 or newer works well for building `net48` projects).
2. Point the build at your RimWorld managed assemblies. You can either:
   ...
   - pass the `RimWorldManagedPath` MSBuild property (e.g. `dotnet build Source/Odyssey_HoloAI.csproj -p:RimWorldManagedPath="/path/to/RimWorldWin64_Data/Managed"`).
...
3. Build the project:
   ...
   ```bash
   dotnet build Source/Odyssey_HoloAI.csproj
   ```
